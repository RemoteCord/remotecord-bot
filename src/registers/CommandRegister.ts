...
export class CommandRegister {
	static async registerCommands(client: DiscordClient) {
		...
		for (const folder of commandFolders) {
			...
			for (const file of commandFiles) {
				...
				const command: Command = new (await import(`file://${commandFolderPath}/${folder}/${file}`)).default();
				if (!command?.name) {
	...
	static async registerSlashApi(client: DiscordClient) {
		const slashCommands = client.commands.filter((command) => command.slash && command.enabled).map((command) => command.slash);
